...
| Per-user/per-session isolation; sanitize before persistence | SoTA §4.4 | MUST | wired-healthy | Per-vessel worktree isolation (`cli/internal/worktree/`) | Every vessel runs in an isolated worktree under `.daemon-root/.xylem/worktrees/` | `[Established]` | (no action) |
