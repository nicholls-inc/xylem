...
`xylem init` scaffolds two standard workflows into your repo: `fix-bug` and `implement-feature`, alongside `.xylem/HARNESS.md` and matching prompt templates. This repository also documents a repo-specific `implement-harness` workflow, but that workflow is checked in here rather than generated by `xylem init`.
