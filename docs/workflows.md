...
`xylem init` scaffolds the core profile into your repo: delivery workflows such as `fix-bug` and `implement-feature`, recurring operator workflows such as `lessons`, `context-weight-audit`, `workflow-health-report`, `security-compliance`, and `auto-triage-issues`, plus `.xylem/HARNESS.md` and matching prompt templates. This repository also documents repo-specific overlay workflows such as `implement-harness`, but those are checked in here rather than generated by the base core profile alone.
...
Runs a repo-scoped scheduled issue-labeling pass in 3 phases: discovery, conservative classification, and label application.

```yaml
name: auto-triage-issues
class: ops
description: "Discover unlabeled issues, classify them conservatively, and apply labels in batch"
phases:
  - name: discover
  ...
  - name: classify
    ...
    gate:
      ...
      run: |
        set -eu
        out=".xylem/phases/{{.Vessel.ID}}/classify.output"
        test -s "$out"
        gh label list --repo "{{.Repo.Slug}}" --limit 200 --json name >/tmp/repo-labels.json
  - name: apply
...
1. **discover** -- Queries GitHub for up to 10 open issues that still have zero labels, captures enough body/author context for later triage, and exits early with `XYLEM_NOOP` when there is nothing to do.
2. **classify** -- Proposes only labels that already exist in the repository, prefers `needs-triage` whenever confidence is below the configured threshold, and passes a command gate that validates the JSON payload shape plus label taxonomy.
3. **apply** -- Re-reads each issue before editing so concurrent human changes win, adds only clearly safe labels, and emits an audit-style report summarizing applied, skipped, and failed edits.

**When to use:** Assign this workflow to a `scheduled` source when you want xylem to keep unlabeled issue backlog moving without auto-closing issues or forcing low-confidence classifications.
